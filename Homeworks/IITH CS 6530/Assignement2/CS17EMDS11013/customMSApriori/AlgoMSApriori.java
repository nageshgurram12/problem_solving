...
public class AlgoMSApriori {
	...
	List<List<Itemset>> canNotBeTogether = null;
	
	List<List<Itemset>> mustBeTogether = null;
	/**
	 * Constructor
	 */
	...
	public void runAlgorithm(String input, String output,
			double beta, double LS, double SPD) throws IOException { 
		runAlgorithm(input, output, beta, LS, SPD, null, null);
	}
	 ...
	 */
	public void runAlgorithm(String input, String output,
			double beta, double LS, double SPD, String canNotBeTogetherFile, String mustBeTogetherFile) throws IOException { 
		...
		if(canNotBeTogetherFile != null){
			canNotBeTogether = new ArrayList<>();
			readConstraints(canNotBeTogether, canNotBeTogetherFile);
		}
		
		if(mustBeTogetherFile != null){
			mustBeTogether = new ArrayList<>();
			readConstraints(mustBeTogether, mustBeTogetherFile);
		}
		...
		this.SPD = (int) Math.ceil(SPD * transactionCount);
		...
		for (Entry<Integer, Integer> entry : mapItemCount.entrySet()) {
			...
			if (entry.getValue() >= MIS[entry.getKey()] && maxItemsetSize >=1) {
				...
				if(checkForMustHaveTogether(new Itemset(new int[]{item}), mustBeTogether)){
		...
		if(maxItemsetSize >1){
			...
			do {
				...
				if (k == 2) {
					...
					candidatesK = generateCandidateSizeK(level, mapItemCount);
				}
				...
				// for each candidate
				candLoop : for (Itemset candidate : candidatesK) {
					...
					if (candidate.getAbsoluteSupport() >= MIS[candidate.get(0)]) {
						// before adding to frequent items list check for item constraints
						if(canNotBeTogether != null){
							for(i = 0; i < canNotBeTogether.size(); i++){
								List<Itemset> itemsets = canNotBeTogether.get(i);
								int itemsetsPresent = 0;
								for(Itemset itemset : itemsets){
									if(isSubset(itemset, candidate)){
										itemsetsPresent++;
										// If more than two cant be together itemsets are present, then its not candidate
										if(itemsetsPresent >= 2){
											continue candLoop;
										}
									}
								}
						...
						if(!checkForMustHaveTogether(candidate, mustBeTogether)){
							continue candLoop;
						}
	...
	/**
	* Check for must be together constraint on candidate 
	*/
	private boolean checkForMustHaveTogether(Itemset candidate,List<List<Itemset>> mustBeTogether){
		if(mustBeTogether != null){
			for(int i = 0; i < mustBeTogether.size(); i++){
				List<Itemset> itemsets = mustBeTogether.get(i);
				for(Itemset itemset : itemsets){
					if(isSubset(itemset, candidate)){
						return true;
					}
	...
	/**
	* check if constraintSet is subset of candidate
	*/
	private boolean isSubset(Itemset constraintSet, Itemset candidate){
	    // first we check the size
		if(candidate.size() < constraintSet.size()){
			return false;
		}
		...
		// for each item in constraintSet, we will try to find it in this itemset
		for(int j =0; j < constraintSet.size(); j++){
			...
			while(found == false && i< candidate.size()){
				// if we found the current item from constraintSet, we stop searching
				if(candidate.get(i).equals(constraintSet.get(j))){
					found = true;
				}// if the current item in this itemset is larger than 
				// the current item from constraintSet, we return false
				// because the itemsets are assumed to be lexically ordered.
				else if(itemComparator.compare(candidate.get(i), constraintSet.get(j)) > 0){
					return false;
				}
	...
	private List<Itemset> generateCandidate2(List<Integer> frequent1, Map<Integer, Integer> mapItemCount) {
		...
		for (int i = 0; i < frequent1.size(); i++) {
			...
			for (int j = i + 1; j < frequent1.size(); j++) {
				...
				if(Math.abs(mapItemCount.get(item1) - mapItemCount.get(item2)) <= SPD){
			...
			loop2: for (int j = i + 1; j < levelK_1.size(); j++) {
				...
				for (int k = 0; k < itemset1.length; k++) {
					...
					if (k == itemset1.length - 1) {
						...
						// check support difference constraint between both the items
						if(Math.abs(mapItemCount.get(itemset1[k]) - mapItemCount.get(itemset2[k])) > SPD){
							continue loop2;
						}
					}
	...
	private void saveItemsetToFile(Itemset itemset) throws IOException {
		...
		writer.write(itemset.toString() + " #SUP: "	+ itemset.getAbsoluteSupport() + " #MIS: " + MIS[itemset.get(0)]);
		writer.newLine();
	...
	private void saveItemsetToFile(Integer item, Integer support)
		...
		writer.write(item + " #SUP: " + support + " #MIS: " + MIS[item]);
		writer.newLine();
		// increase the number of itemsets found
		itemsetCount++;
	}
	...
	/**
	* Read constrainsts from given file and prepare list
	*/
	private void readConstraints(List<List<Itemset>> constraintList, String constraintFile) throws IOException{
		BufferedReader constraintsReader = new BufferedReader(new FileReader(constraintFile));
		
		String line;
		while((line = constraintsReader.readLine()) != null){
			// if the line is  a comment, is  empty or is a
			// kind of metadata
			if (line.isEmpty() == true ||
					line.charAt(0) == '#' || line.charAt(0) == '%'
			...
			String[] lineSplited = line.split(",");
			List<Itemset> canNotBeTogetherItemsets = new ArrayList<>();
			for(String itemsetStr: lineSplited){ 
				...
				canNotBeTogetherItemsets.add(new Itemset(items));
			}
			if(canNotBeTogetherItemsets.size() > 0){
				constraintList.add(canNotBeTogetherItemsets);
			}
		}
