...
ada_optimizer = optim.Adagrad(model.parameters(), lr=ada_lr, lr_decay=ada_lr_decay, weight_decay=weight_decay)
lbfgs_optimizer = optim.LBFGS(model.parameters(), lr=lbfgs_lr);

#Default SGD optimizer
sgd_optimizer = optim.SGD(model.parameters(), lr=0.1, weight_decay=0.01)

if optim_algo == "Adagrad":
  optimizer = ada_optimizer
elif optim_algo == "LBFGS":
