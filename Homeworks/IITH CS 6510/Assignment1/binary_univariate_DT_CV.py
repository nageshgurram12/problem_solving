...
class DecisionTree():
  ...
  def stop_criteria(self, features, dataset, initial_impurity):
    ...
    1) initial_impurity <= LEAF_THRESHOLD_ENTROPY
    2) dataset < LEAF_DATASET_THRESHOLD_COUNT
    '''
    if initial_impurity <= LEAF_THRESHOLD_ENTROPY:
      return True
...
def run_decision_tree():
  ...
  for itr in range(K):
    training_set = np.array([x for i, x in enumerate(data) if i % K != itr], dtype=float)
    test_set = np.array([x for i, x in enumerate(data) if i % K == itr], dtype=float)
